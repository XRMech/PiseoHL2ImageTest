...
namespace Microsoft.MixedReality.Toolkit.Utilities
{
    ...
    public abstract class ClippingPrimitive : MonoBehaviour, IMaterialInstanceOwner
    {
		...
		{
			Debug.Log("SET RENDERERS");
			renderers = FindObjectsOfType<Renderer>().ToList();
			renderers.Remove(GetComponentInChildren<Renderer>());
			for (int i = 0; i < renderers.Count; i++)
			{
				//Debug.Log(renderers[i].material.shader.name);
				if (!renderers[i].sharedMaterial.shader.name.Contains("Mixed Reality Toolkit"))
					renderers.RemoveAt(i);
				//else Debug.Log("KEEP THIS");
			}
		}
		...
		private IEnumerator WaitingToRefresh()
		{
        ...
        {
            Initialize();
			//if (UnityEngine.SceneManagement.SceneManager.GetActiveScene().buildIndex >= 0) SetRenderers();
            UpdateRenderers();
            //ToggleClippingFeature(true);
			StartCoroutine(WaitingToRefresh());
        ...
        public void ToggleClippingFeature(bool keywordOn)
        {
            if (renderers != null)
            {
                for (var i = 0; i < renderers.Count; ++i)
        ...
        public void ToggleClippingFeature(Material[] materials, bool keywordOn)
        {
            if (materials != null)
            {
